...
var operateText = (function ($) {
    ...
    */
    ...
    function InsertTag(type, span, offset, offsetS, offsetE, tag) {
        ...
        if (copyTag.hasClass('double')) {
            ...
            tempTag.find('img').attr('src', '/public/images/tag-' + tempTag.attr('data-no') + '-02.png');
            copyEndTag = tempTag;
        };
    ...
    */
    ...
    function insertTagEntry(type, $tr, tag) {
        ...
        // editTd = $(range.commonAncestorContainer).parents('td.active-text');
        ...
        // && (range.commonAncestorContainer.className == 'edition-target' || range.commonAncestorContainer.className == 'edition-source') ? editTd[0].parentNode : editTd[0].parentNode;
        //选中原文，不能插入
        ...
        for (var n = 0; n < nodes.length; n++) {
            if (nodes[n].nodeName == 'SPAN' && $(nodes[n]).hasClass('tagWrap')) {
                if ($('script.current').attr('data-lan')) {
                    $.Alert('Failed to insert the tag');
                } else {
                    $.Alert('插入标签失败');
                }
                return;
        ...
        if ($(range.startContainer).hasClass('tagWrap')
            && $(range.startContainer).hasClass('start')
    ...
    */
    function copyStyleEntry(ele) {
        ...
        if ($(change).parents('tr').hasClass('locked') || $(change).parents('tr').hasClass('repeated') || !$(change).parents('td').hasClass('target-text')) {
            return false;
        }
