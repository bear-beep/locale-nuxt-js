...
var dealCatObj = (function (window, document, $) {
    ...
    */
    ...
    function insertCharacter(_this,word){
        ...
        if (userSelection.type === 'Caret'
            ...
            var txtNode = document.createTextNode(word);//创建文本节点
            (targetEl.hasClass('tagWrap') || targetEl.hasClass('edition-target'))
            && (txtNode = document.createTextNode(word));
            range.insertNode(txtNode);//插入文本节点
            newRange = document.createRange();
            newRange.selectNodeContents(txtNode);
            range.setStart(txtNode, txtNode.length);
    ...
    */
    ...
    function mtShow(data,txt,senId,fid,wid){
        ...
        data.data.translation = data.data.translation.replace(/</g, "&lt;").replace(/>/g, "&gt;");
        txt = txt.replace(/</g, "&lt;").replace(/>/g, "&gt;");
        ...
        html = `<tr class="key-copy" toolmt="true">
                            ...
                            <td class="catTrans trans" data='${senId}' data-fid='${fid}' data-wid='${wid}'>${data.data.translation}</td>
                            <td style="display:none">${data.data.translation}</td>
                        </tr>`
    ...
    function submitStarScore (type) {
        ...
        if (type === 1){
            ...
            // });
            // if (!isChecked){
            //     $.Alert('Select at least one error type');
            //     return
            // }
                ...
                if (res.success) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        for (var i = 0, len = data.length; i < len; i++) {
                            ...
                            let isTitle = data[i].reverse ? "title='From reverse corpus'" : "",
                        ...
                        $(".tmTrans>tbody").html(`<div style="height: 90px;display:flex;justify-content:center;align-items:center;text-align: center;font-size: 12px;color: #999;">No data</div>`);
                    }
                    BOM.setTransHeight();
                }
            },
    ...
    */
                ...
                if (data && data.length > 0){
                    ...
                    for (var i = 0, len = data.length; i < len; i++) {
                        var temp = data[i].Model,
                            ...
                            isTitle = data[i].IsProj ? "title='PO terminology'" : "";
                    ...
                    $("table.transList>tbody").html(`<div style="height: 90px;display:flex;justify-content:center;align-items:center;text-align: center;font-size: 12px;color: #999;">No data</div>`);
                }
            }
        });
    }
    ...
    function showDetailTBTM(_this) {
        ...
        if ($(_this).hasClass('tr-xm')) {// 搜索处项目库
            ...
            ul.show();
            ul.find('i.tmtbCreater').html('Submitted by：' + list.name).attr('title', list.name);
            ul.find('i.tmtbArea').html('Source ：' + list.area).attr('title', list.area);
            ul.find('i.tmtbTime').html('Time of submission：' + list.time).attr('title', list.time);
            ...
            $.get(_url, function (res, text, xhr) {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    ...
                    if (_this.dataset.istrue && _this.dataset.istrue == "true"
                        ...
                        ul.find('i.tmtbArea').html('Source ：Project TM').attr('title', 'Project TM');
                        ul.find('i.tmtbCreater').html(`Submitted by：${data.Modifier} <span style="margin-left: 10px;"> ${timeFormat(data.ModifiedTime)} </span>`).attr('title', data.Modifier + timeFormat(data.ModifiedTime));
                        // ul.find('i.tmtbTime').html('Time of submission：' + new Date(data.ModifiedTime).toLocaleString('chinese',{hour12:false})).attr('title', new Date(data.ModifiedTime).toLocaleString('chinese',{hour12:false}));
                        ul.find('i.tmtbTime').html('');
                        ul.find('i.tmtbRemark').html('Notes：' + data.Remark + ';' + data.note).attr('title', data.Remark + ';' + data.note);
                    } else {
                        $.get(urls.tmtbUrl + "/" + type[1] + "/" + id + "?rowKey=" + key, function (res1, text, xhr) {
                            if (xhr.readyState === 4 && xhr.status === 200) {
                                var result = res1.data;
                                if (div.hasClass('tmContent')) {// 语料处
                                    let currentTd = `<td style="width: 23.5%;"><a class="tmOperate" href="javascript:$('.update-tmtb-content').remove();$(document.body).append($('#updateTMTB').html());$('.modal-tm').css('display','block');$('.last-origin').html($('#lastOrigin').text());$('.update-orin').html($('#inputCurren').text());$('.next-origin').html($('#nextOrigin').text());$('.update-trans').html($('#inputTrans').text());$('#updateTmTbData').attr('data-mold','${mold}');event.preventDefault();" data-key="${key}" data-creater="${data.Creator}" data-databaseId="${data.DatabaseId}">Update TM</a></td>`
                                    if (_this.dataset.reverse === 'true') {
                                        currentTd = `<td style="width: 23.5%;color: #ccc;">Update TM</td>`
                                    }
                                    var tdStr = `<tr>
                                                    ${currentTd}
                                    ...
                                    $('.tmtbOperate').css('display',professional==='true'?'none':'inline');
                                    ul.find('i.tmtbArea').html(`Name： ${type[1] === 'tmindexinfo' ? result.field : result.Field}`).attr('title', `${type[1] === 'tmindexinfo' ? result.field : result.Field}`);
                                    ul.find('i.tmtbCreater').html(`Submitted by：${type[1] === 'tmindexinfo' ? result.modifier : result.ModifiedInformation.Modifier}<span style="margin-left: 10px;">${timeFormat(type[1] === 'tmindexinfo' ? result.modifiedTime : result.ModifiedInformation.ModifiedTime)}</span>`).attr('title', `${type[1] === 'tmindexinfo' ? result.modifier : result.ModifiedInformation.Modifier}` + ' ' + timeFormat(type[1] === 'tmindexinfo' ? result.modifiedTime : result.ModifiedInformation.ModifiedTime));
                                    // ul.find('i.tmtbTime').html('Time of submission：' + new Date(result.ModifiedInformation.ModifiedTime).toLocaleString('chinese',{hour12:false})).attr('title', new Date(result.ModifiedInformation.ModifiedTime).toLocaleString('chinese',{hour12:false}));
                                    ul.find('i.tmtbTime').html('');
                                    ul.find('i.tmtbRemark').html(`Notes：${type[1] === 'tmindexinfo' ? result.remark + ';' + result.note : result.ModifiedInformation.Remark + ';' + result.ModifiedInformation.note}`).attr('title', `${type[1] === 'tmindexinfo' ? result.remark + ';' + result.note : result.ModifiedInformation.Remark + ';' + result.ModifiedInformation.note}`);
                                }
                            }
                        });
                    }
    ...
    function tempUpdateTBTM(_this) {
        ...
        $.ajax({
            ...
            success: function (data, text, xhr) {
                ...
                if (data.success) {
                    $.Tips('Modification successful');
                    setTimeout(() => {
                        if (url === "/tm"){
                            let fill = false,
                                tr = $('tr.table-row.active'),
                                fid = tr.attr("data-fid"),
                            ...
                            dealCatObj.getTMofResult(senid, fid, dataConfig.DataBaseId_TM, dataConfig.percentge, dataConfig.number, params.CurrentOrigin, prevTxt, nextTxt, tr,xlid,fill);
                        }
                    },500)
                } else {
                    $.Alert(data.msg);
    ...
    function addTerm(_this) {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $(_this).attr("disabled", true);
                $(_this).html('Saving<i class="am-icon-spinner am-icon-spin"></i>');
            },
            complete: function () {
                $(_this).removeAttr("disabled");
                $(_this).html("Add");
...
var dealQaObj = (function (window, document, $) {
    ...
    function checkWordAndSpace(_this, btn, list) {
        ...
        for (var i = 0, len = list.length; i < len; i++) {
            ...
            // tempobj.status = true;
            // tempobj.filterItem = list[i].value;
            // tempobj.langType = dataConfig.LangTypePair;
            ...
            // params.filterItemList.push(tempobj);
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $(_this).attr("disabled", true);
                $(_this).html('Adding<i class="am-icon-spinner am-icon-spin"></i>');
            },
            complete: function () {
                $(_this).removeAttr("disabled");
                $(_this).html("Confirm");
            ...
            success: function (data) {
                if (data.success) {
                    $.Tips("Added successfully");
                    $(".full-content").find("a.am-close").click();
                    // 修改
                    getSensitiveWord();
                    // 修改结束
                } else {
                    $.Alert(data.errorsEnglish);
                }
            }
        });
    }
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    if (data.data.length > 0) {
                        ...
                        _title = 'Sensitive words：' + arr.join(';');
    ...
    function getQASet(type) {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $(_this).attr("disabled", true);
                $(_this).html('Initialize<i class="am-icon-spinner am-icon-spin"></i>');
                $.LoadTips('Loading');
            },
            complete: function () {
                $(_this).removeAttr("disabled");
                $(_this).html('<i class="am-icon-cog"></i> QA settings');
                $(".my-loading").remove();
            },
            success: function (data) {
                if (data.success) {
                    ...
                    for (var i = 0, len = data.data.length; i < len; i++) {
                        ...
                        for (var j = 0, jLen = params.length; j < jLen; j++) {
                            var opt = params[j];
                            if ($(opt).attr("class") == _name && _name !== 'MatchingRateCheck' && _name !== 'SpaceBeforePuncCheck') {
    ...
    $('input[name=setMatchQA]').blur(function () {
        var matchQA = $('input[name=setMatchQA]');
        if (matchQA.val() < 76) {
            $.Alert('Match rate needs to be above 76%！');
            matchQA.val(76);
            return;
        }
        if (matchQA.val() > 101) {
            $.Alert('Match rate needs to be below 101%！');
            matchQA.val(101);
            return;
        }
    });
    ...
    $("#qaRunBtn,span.qa-sort").on("click", function () {
        ...
        if (_this.classList.contains('qa-sort')) {
            if (sort == '0') {
                ...
                _this.innerHTML = 'SN <i class="am-icon-caret-down"></i>';
            } else {
                _this.setAttribute('data', '0');
                _this.innerHTML = 'SN <i class="am-icon-caret-up"></i>';
            }
            sort = _this.getAttribute('data');
        }
        $.LoadTips('QA checking');
        var fileIdAndPackId = {};
        for (let i = 0; i < dataConfig.dataList.length; i++) {
            var packid = dataConfig.dataList[i].Packid;
            fileIdAndPackId[dataConfig.dataList[i].Fileid] = packid;
        ...
        $.ajax({
            ...
            beforeSend: function () {
                if (_this.id === 'qaRunBtn') {
                    ...
                    $("#qa_page_num,#qaRunBtn").attr("disabled", true);
                    $("#qa_count").html('total -- entries ');
                    $(_this).html('Checking<i class="am-icon-spinner am-icon-spin"></i>');
                }
            ...
            complete: function () {
                if (_this.id === 'qaRunBtn') {
                    ...
                    $("#qa_page_num").removeAttr("disabled", true);
                    $(_this).html("QA check");
                }
                $(".qa-isIgnore,#qaRunBtn").removeAttr("disabled");
                $(".my-loading").remove();
                $('input[name="qaCheckList"]').removeAttr('checked');
            },
            success: function (data) {
                if (data.success) {
                    data.data.length === 0 && $.Tips("No QA data");
                    var listStr = "";
                    for (var i = 0, iLen = data.data.length; i < iLen; i++) {
                        ...
                        listStr += `<tr class='qa-tr' tooltips='${data.data[i].checkName}' data-id="${data.data[i].sentenceId}" data-sen='${data.data[i].sen}' data-json='${jsonStr}' data-fid='${data.data[i].fileId}' data-comparFileId='${data.data[i].comparFileId}'>
                                        <td class="clickTd" width='38'><input type="checkbox" name="ingoreCheck" ${(data.data[i].ignoreFlag === 0)?"disabled=true'":''} value='${data.data[i].detailName}' data-checkParam="${data.data[i].checkName}"  data-check="${data.data[i].checkParam}" data-id="${data.data[i].sentenceId}" data-fid='${data.data[i].fileId}' data-pid='${data.data[i].packId}' data-fileName="${data.data[i].filename}" data-checkLevel="${data.data[i].checkLevel}" /></td>
                    ...
                    // QA检查分页
                    $("#qa_count").html('total ' + data.total + ' entries ');
                    ...
                    $.Alert(data.msgEnglish);
                }
            },
            error:function(err){
                $('.my-loading').remove();
                err.responseJSON && $.Alert(err.responseJSON.msgInEnglish);
            }
        })
    });
            ...
            success: function (data) {
                if (data.data && data.data.length > 0){
                    ...
                    for (var i = 0, iLen = data.data.length; i < iLen; i++) {
                        ...
                        // 含有是否忽略
                        listStr += `<tr class='qa-tr' tooltips='${data.data[i].checkName}' data-id="${data.data[i].sentenceId}" data-sen='${data.data[i].sen}' data-json='${jsonStr}' data-fid='${data.data[i].fileId}' data-comparFileId='${data.data[i].comparFileId}'>
                                        <td class="clickTd" width='38'><input type="checkbox" name="ingoreCheck" ${(data.data[i].ignoreFlag === 0)?"disabled=true'":''} value='${data.data[i].detailName}' data-checkParam="${data.data[i].checkParam}"  data-check="${data.data[i].checkName}" data-id="${data.data[i].sentenceId}" data-fid='${data.data[i].fileId}' data-fileName="${data.data[i].filename}" data-checkLevel="${data.data[i].checkLevel}" /></td>
                                        <td class="clickTd" width='55'>${data.data[i].sentenceId}</td>
                                        <td class="clickTd" width='55' title="${data.data[i].filename}" style="overflow:hidden;white-space: nowrap;text-overflow: ellipsis;">${data.data[i].filename}</td>
                                        <td class='qa-select clickTd'>${data.data[i].checkName}</td>
                                        <td class="clickTd" title='${data.data[i].detailName}' style="min-width: 70px;overflow:hidden;white-space: nowrap;text-overflow: ellipsis;">${data.data[i].detailName}</td>
    ...
    $('input[name="ignore"],input[name="noIgnore"]').on('click', function () {
        if (!$("input[name=ignore]").is(":checked") && !$("input[name=noIgnore]").is(":checked")){
            $.Tips("Select at least one QA screen");
    ...
    $('#qaIgnoreBtn').on('click', function () {
        ...
        if (checkedBox.length < 1) {
            $.Tips('Items to be ignored not selected');
            return;
        }
        var allMsg = [];
        var ignoreList = [];
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $(_this).attr("disabled", true);
                $(_this).html('Ignoring<i class="am-icon-spinner am-icon-spin"></i>');
            },
            complete: function (xhr) {
                $(_this).removeAttr("disabled");
                $(_this).html('Ignore in batch');
    ...
    */
        ...
        $.ajax({
            ...
            success: function (data, text, xhr) {
                ...
                if (data.success && data.data.length > 0) {
                    ...
                    $(".trans_spell").html(`<div style="height: 90px;display:flex;justify-content:center;align-items:center;text-align: center;font-size: 12px;color: #999;">No data</div>`);
                }
            }
        });
    }
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    if (res.data.length > 0) {
                        ...
                        $.Alert("No assignment in the process！");
    ...
    function submitEvaluateFnc(_that, subType) {
        ...
        if (subType == '提交') {
            if (checkData == '' || checkData == undefined || checkData == null) {
                $.Alert('Select the problem involved');
                return
            }
            ...
            params.typeName = $("input[name='errorTypeMsg']:checked").attr("dataType");
        } else {
        ...
        $.ajax({
            ...
            data: JSON.stringify(params),
            beforeSend: function () {
                $.LoadTips('Evaluation in progress');
            },
            success: function (data) {
                if (data.success) {
                    $(".my-loading").remove();
                    closeEvaluateBox();
                    $.Tips('Evaluation successful');
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    var html = "";
                        html += `
                                <span>Words evaluated：${data.data.wordsAmount}</span>
                                <span style="margin-left: 20px;">Segments evaluated：${data.data.sentenceEvaed}</span>
                                <!--<span style="margin-left: 20px;">Segments to be evaluated：${data.data.sentenceLeft2Eva}</span>-->
                                <span class="am-icon-question-circle evaluateBtnExplain" onclick="$('.evaluationExplainBtn').click()"></span>`;
                    var link = $('script.current').attr('data-link');
                    if (link !== 'ES' && link !== 'T') {
                        container.innerHTML = html;
                    }
                    $('.evaluateSen1').html('Segments evaluated：' + data.data.sentenceEvaed);
                }
            }
        });
        $.ajax({
                ...
                if (res.success) {
                    ...
                    if (res.data.length > 0) {
                        res.data.forEach((item, index) => {
                            var str = '/';
                            if (item.description) {
                                str = `<span style="color: #159396;" title="${item.description}">View</span>`;
                            } else {
                                str = '/';
                            }
                            html += `<tr>
                        ...
                        html = `<tr>
                                    <td colspan="7">No data for the moment</td>
                                </tr>`;
                    };
                    if (res.data1.length > 0) {
                        ...
                        html1 = `<tr>
                                    <td colspan="8">No data for the moment</td>
                                </tr>`;
                    }
                    container.innerHTML = html;
    ...
    function openEvaluatePopup(that) {
        ...
        var htmlBtn = `
                ...
                data-sou="${sou}" data-tar="${tar}" data-fid="${fid}" data-pid="${pid}" data-wid="${wid}" data-className="${className}"
                onClick="dealQaObj.submitEvaluateFnc(this, '正常')">Normal</button>
                <button id="evaluateSubBtn" class="am-btn normal-btn" style="background: #159396;color: #fff;" 
                data-sou="${sou}" data-tar="${tar}" data-fid="${fid}" data-pid="${pid}" data-wid="${wid}" data-className="${className}"
                onClick="dealQaObj.submitEvaluateFnc(this, '提交')">Submit</button>
                <button id="evaluateCancelBtn" class="am-btn normal-btn normal-bg-btn" style="" onClick="dealQaObj.closeEvaluateBox()">Cancel</button>`;
            ...
            if (dataConfig.isDTPevaluate) {
                if (className === 'edition-source') {
                    if (dataConfig.userIdDTP !== null) {
                        ...
                        $('.evaluateSen').html('Current segment：' + wid);
                        $('.evaluate_box_bottom').html(htmlBtn);
                    } else {
                        $.Alert('No DTP accountable, project cannot be evaluated！');
                    }
                }
            }
            if (className === 'edition-target') {
    ...
    // 监听输入框字数
    (function () {
        $('#textareaSource,#textareaTarget').on('keyup',function () {
            let len = $(this).val().length;
            if (len > 100) {
                return false;
            }
            $(this).next().find('i').html(len);
        });
        // var text = $('#textareaTarget').val();
        // if (!text) {
        //     return
        // }
        // var len = text.length;
        // $('#textareaTarget').next().find('i').html(len);
        // $('#textareaTarget').keyup(function () {
        //     var text = $(this).val();
        //     len = text.length;
        //     if (len > 100) {
        //         return false;
        ...
        //     $(this).next().find('i').html(len);
        // })
        // var text1 = $('#textareaSource').val();
        // if (!text1) {
        //     return
        // }
        // var len1 = text1.length;
        // $('#textareaSource').next().find('i').html(len1);
        // $('#textareaSource').keyup(function () {
        //     var text1 = $(this).val();
        //     len1 = text1.length;
        //     if (len1 > 100) {
        //         return false;
        //     }
        //     $(this).next().find('i').html(len1);
        // })
    })();
    ...
    function mustEvaFuc(UserID, packID, type) {
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    if (!data.data) {
                        let str = `<span class="am-icon-info-circle evaluateIsMust">This task is optional for evaluation</span>`
    ...
    function addDictionary(sid, word, fid) {
        ...
        $.Confirm('Are you sure to add it to dictionary?');
        $('#my-confirm').modal({
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success && data.data && data.data.length > 0) {
                    ...
                    $('.update_insert').before('<font class="new-message">You have a new message, click to view（' + data.data.length + '）</font>');
    ...
    function getHistoryDetail() {
        ...
        for (var i = 0, len = tarList.length; i < len; i++) {
            tarList[i].onmousedown = function () {
                ...
                _parent.next('ul').find("i.updateUser").html("Modified by：" + $data.user).attr('title', $data.user);
                _parent.next('ul').find("i.updateTime").html("Time of modification：" + $data.time).attr('title', $data.time);
                _parent.next('ul').find("i.updateNode").html("Modification node：" + $data.node).attr('title', $data.node);
                _parent.next('ul').find("i.updateInfo").html("Notes：" + $data.remark).attr('title', $data.remark);
            }
        }
    }
    ...
    */
    ...
    function referSen(flag) {
        ...
        if (selectedTr.length < 1) {
            $.Alert('Segments not checked (please click on the segment number)');
            return;
        }
        var params = [];
        ...
        $.LoadTips(flag ? 'Batch submitting' : 'Batch canceling');
        $.ajax({
            method: "post",
            ...
            success: function (data) {
                $(".my-loading").remove();
                if (data.success) {
                    $.LoadTips(`${flag ? 'Submitted' : 'Cancelled'}，Refreshing`);
                    window.setTimeout(() => {
                        // localParam.pageNum = Number($(selectedTr[0]).attr('data-page'));
                        localParam.isFirst = true;
                        // loadPage.initPage($(selectedTr[0]).attr('data-no'), $(selectedTr[0]).attr('data-fid'),'','batch');
    ...
    function confirmPackTask(_this) {
        ...
        if (typemethod === 'SubmitES') {
            $.Confirm('Please verify that all segments are confirmed in case they are not saved after submission');
        } else {
            $.Confirm('Are you sure to submit all targets?');
        }
        $('#my-confirm').modal({
            onConfirm: function (e) {
                if (typemethod === 'SubmitES') {
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        $(_this).attr("disabled", true);
                        $(_this).html('Submitting<i class="am-icon-spinner am-icon-spin"></i>');
                    },
                    complete: function () {
                        $(_this).removeAttr("disabled");
                        if (typemethod === 'SubmitES') {
                            ...
                            $(_this).html("Submit all");
                        }
                    },
                    success: function (data) {
                        $(".my-loading").remove();
                        if (data.success) {
                            $.LoadTips('Submission successful. Refreshing');
                            window.setTimeout(() => {
                                localParam.isFirst = true;
                    ...
                    error:function(err){
                        $('.my-loading').remove();
                        $.Alert(err.responseJSON.msgInEnglish);
                    }
                });
            },
            onCancel: function (e) {
    ...
    function preTranLibrary(_this) {
        ...
        $.Confirm('Are you sure to populate TM?');
        $('#my-confirm').modal({
            onConfirm: function (e) {
                $.LoadTips('Populating TM');
                $.ajax({
                    method: "post",
                    ...
                    beforeSend: function () {
                        $(_this).attr("disabled", true);
                        $(_this).html('Populating TM<i class="am-icon-spinner am-icon-spin"></i>');
                    },
                    complete: function () {
                        $(_this).removeAttr("disabled");
                        $(_this).html("Apply TM");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                        ...
                        case 'T':
                            isConfirm === '0' ? (
                                $(tr).attr("data-true", "ok"),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "1", "title": "Translated"}),
                                !alt && (
                                ...
                                $(tr).attr("data-true", ""),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "0", "title": "Untranslated"})
                            );
                            break;
                        case 'C':
                            +isConfirm < 3 ? (
                                $(tr).attr("data-true", "ok"),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "3", "title": "Reviewed"}),
                                !alt && (
                                    $(tr).addClass("enterTag").siblings().removeClass("enterTag"),
                                        classOperation.scrollToNoConfirmSentence($(tr))
                                )
                                ...
                                $(tr).attr("data-true", ""),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "2", "title": "Pending review"})
                            );
                            break;
                        case 'Q':
                            +isConfirm < 5 ? (
                                $(tr).attr("data-true", "ok"),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "5", "title": "QA completed"}),
                                !alt && (
                                    $(tr).addClass("enterTag").siblings().removeClass("enterTag"),
                                        classOperation.scrollToNoConfirmSentence($(tr))
                                )
                                ...
                                $(tr).attr("data-true", ""),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "4", "title": "Pending QA"})
                            );
                            break;
                        case 'P':
                            +isConfirm > -2 ? (
                                $(tr).attr("data-true", "ok"),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "-2", "title": "Reviewed by PM"}),
                                !alt && (
                                    $(tr).addClass("enterTag").siblings().removeClass("enterTag"),
                                        classOperation.scrollToNoConfirmSentence($(tr))
                                )
                                ...
                                $(tr).attr("data-true", ""),
                                    ...
                                    $(tr).find("td.confirm").attr({"data-iscon": "-1", "title": "Being revised by PM"})
                            );
                            break;
                        case 'ES':
                            isConfirm === '0' ? (
                    ...
                    $.Confirm(data.msgInEnglish);
                    $('#my-confirm').modal({
                        onConfirm: function (e) {
                            const newParams = {};
                            Object.assign(newParams, params, {GoSubSen: "-1"});
                            $.ajax({
                                method: "post",
                                url: urls.editUrl + "/" + typeofmethod + "/SubSen",
                                dataType: "json",
                                data: {
                                    ...
                                    JsonTrans: JSON.stringify(dealTextObj.styleStringToList(targetDiv))
                                }
                            })
                        },
                        onCancel: function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (!data.success && data.data !== '-2' && $.trim(tarDiv.text()) !== "") {
                    ...
                    $(".remarkTxt").val(new Date().toLocaleString('chinese',{hour12:false}) + " Target submission failed");
    ...
    function lockSentence(_this) {
        ...
        if (link !== "P") {
            $.Alert("No permission to operate");
            return;
        }
        var params = {
            CreatedTime: dataConfig.createdTime,
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    isLock == 'false' ? (
                        $(_this).attr({"data-islock": "true", "title": "Locked"})
                    ) : (
                        $(_this).attr({"data-islock": "false", "title": "Unlocked"})
                    );
                } else {
                    $.Alert(data.msgInEnglish);
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success && data.data != null) {
                    dataConfig.allSubmitTime = Math.ceil(data.data.originalSentenceNumber/0.2);
                    //悬浮显示文件名
                    let fileName = data.data.fileName.replace(/;/g,`<br>`);
                    $(".file_name").html(dataConfig.dataList.length > 1 ? 'FileName：' + dataConfig.dataList.length + ' files' : 'FileName：' + data.data.fileName).on('mouseenter',function () {
                        $(".popup").html(fileName);
                        $(".popup,.popupTriangle").css('display','block');
                    }).on('mouseleave',function () {
                        $(".popup,.popupTriangle").css('display','none');
                    });
                    ...
                    // $(".file_name").attr('title', data.data.fileName).html('FileName：' + data.data.fileName);
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    $("#previewCont").html('preview failed, please try again');
                    $.Alert(data.msgInEnglish);
                }
            },
            error: function () {
    ...
    function previewTranslation() {
        ...
        $.ajax({
            ...
            success: function (data) {
                data.success ? window.open(urls.editUrl + "/ViewFilePDForHTML/" + data.data, '_blank') : $.Alert(data.msgInEnglish);
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success && data.data.length > 0) {
                    ...
                    $(".update_insert").after(`<div class="transTab" style="height: 90px;display:flex;justify-content:center;align-items:center;text-align: center;font-size: 12px;color: #999;">No data</div>`);
                }
            }
        })
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success && data.data > 0) {
                    ...
                    $(".read_insert").after(`<div class="transTab" style="height: 90px;display:flex;justify-content:center;align-items:center;text-align: center;font-size: 12px;color: #999;">No data</div>`);
                }
            }
        })
    };
    ...
    function refuseTask([link, packid, senid, fid, sign]) {
        $.Confirm('Are you sure to reject checked segments?');
        $('#my-confirm').modal({
            onConfirm: function (e) {
                if (link && packid && senid) {
                    $.LoadTips("Rejecting checked segments");
                    let _params = {
                        RefuseType: 'S',
                        Link: link,
                        PackID: packid,
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            $(".my-loading").remove();
                            if (data.success) {
                                $.LoadTips("Reject successful, refreshing");
                                window.setTimeout(() => {
    ...
    function showRemarkModal(_this,foco) {
        ...
        //     $.Alert('This segment cannot be operated');
        //     return;
        // }
    ...
    function queryRemark(fileID,packID,wordId) {
        ...
        $("#remarksColumn").html('Current paragraph：'+wordId);
        let _params ={
            "FileID": fileID,
            "PackID": packID,
    ...
    function remarkList(wordId,name,link,msg,time,RemarkStatus,FileID,PackID) {
        ...
        let userRole = '';
        switch (link) {
            case "T":userRole = 'translate';break;
            case "C":userRole = 'reviser';break;
            case "P":userRole = 'PM';break;
            case "Q":userRole = 'quality testing';break;
        }
        if (RemarkStatus == 2){
            if (role === link){
                return `<div class="remarkList"><div class="remarkMsg"><p class="current">${msg}</p><span class="operation"><img src="/public/images/right-area/update.png" title="update" alt="" onclick="dealTranObj.updateRemark(${wordId},${RemarkStatus},'${FileID}','${PackID}',this)" /> | 
                <img src="/public/images/right-area/del.png" title="delete" alt="" onclick="dealTranObj.delRemark(${wordId},'${FileID}','${PackID}')" /></span><span class="preservation"><img src="/public/images/right-area/submit.png" class="remarkOk"  title="confirm" alt="" /> | 
                <img src="/public/images/right-area/cancel.png" class="remarkCancel" title="cancel" alt="" /></span></div><div class="role"><span>Add people：${userRole}-${name}<span class="piZhu">comments</span></span><span>Add time：${time}</span></div></div>`
            }else if (dataConfig.isDeleteRemark){
                return `<div class="remarkList"><div class="remarkMsg"><p class="current">${msg}</p><span class="operation"><img src="/public/images/right-area/del.png" title="delete" alt="" onclick="dealTranObj.delRemark(${wordId},'${FileID}','${PackID}','${link}',this)" /></span></div><div class="role"><span>Add people：${userRole}-${name}<span class="piZhu">comments</span></span><span>Add time：${time}</span></div></div>`
            }else {
                return `<div class="remarkList"><div><p >${msg}</p></div><div class="role"><span>Add people：${userRole}-${name}<span class="piZhu">comments</span></span><span>Add time：${time}</span></div></div>`
            }
        }else {
            if (role === link){
                return `<div class="remarkList"><div class="remarkMsg"><p class="current">${msg}</p><span class="operation"><img src="/public/images/right-area/update.png" title="update" alt="" onclick="dealTranObj.updateRemark(${wordId},${RemarkStatus},'${FileID}','${PackID}',this)" /> | 
                <img src="/public/images/right-area/del.png" title="delete" alt="" onclick="dealTranObj.delRemark(${wordId},'${FileID}','${PackID}')" /></span><span class="preservation"><img src="/public/images/right-area/submit.png" class="remarkOk"  title="confirm" alt="" /> | 
                <img src="/public/images/right-area/cancel.png" class="remarkCancel" title="cancel" alt="" /></span></div><div class="role"><span>Add people：${userRole}-${name}</span><span>Add time：${time}</span></div></div>`
            }else if (dataConfig.isDeleteRemark){
                return `<div class="remarkList"><div class="remarkMsg"><p class="current">${msg}</p><span class="operation"><img src="/public/images/right-area/del.png" title="delete" alt="" onclick="dealTranObj.delRemark(${wordId},'${FileID}','${PackID}','${link}',this)" /></span></div><div class="role"><span>Add people：${userRole}-${name}</span><span>Add time：${time}</span></div></div>`
            }else {
                return `<div class="remarkList"><div><p >${msg}</p></div><div class="role"><span>Add people：${userRole}-${name}</span><span>Add time：${time}</span></div></div>`
            }
        }
    }
    //删除备注
    function delRemark(wordId,FileID,PackID,link,_this) {
        $.Confirm('Are you sure to delete the notes?');
        $('#my-confirm').modal({
            onConfirm: function (e) {
                        ...
                        if (data.success){
                            ...
                            if ($("#remarkList div").hasClass('remarkList')){
                                if ($("#remarkList span").hasClass('piZhu')){
                                    ...
                                    currentDiv.attr({"title": "Export as comments", 'data-sts': '2'});
                                }else {
                                    currentTd.removeClass("solve").addClass("unsolve");
                                    currentDiv.attr({"title": "View notes", 'data-sts': '1'});
                                }
                            }else {
                                currentTd.removeClass("solve unsolve");
                                currentDiv.attr({"title": "No notes", 'data-sts': '0'});
                            }
                        }else {
                            $.Tips(data.msgInEnglish);
                        }
                    ...
                    for (var t = 0, len = tempList.length; t < len; t++) {
                        if (org == tempList[t].dataset.no && fid === tempList[t].dataset.fid) {
                            ...
                            if ($target.attr('data-sts') == 2){
                                $target.attr({"title": "Export as comments", 'data-sts': '2'});
                                ...
                                doc.checked ? (
                                    ...
                                    $target.attr({"title": "View notes", 'data-sts': '1'}),
    ...
    function joinFrameSentence() {
        ...
        $.Confirm('Are you sure to merge segments?');
        $('#my-confirm').modal({
            onConfirm: function (e) {
                $.LoadTips("Merging segments");
    ...
    function splitSentence(_this) {
        ...
        var link = $('script.current').attr('data-link'),
            tar = $('tr.table-row.active');
        if (tar.length < 1) {
            $.Alert('Please activate the segment');
            return;
        }
        ...
        if (isDiv.attr('contenteditable') !== 'true') {
            $.Alert("Please make the source editable");
            return;
        }
        ...
        $.LoadTips("Splitting the segment");
        loadPage.splitAndMerge({
            url: url,
            param: params,
            btntype: btntype,
    ...
    function joinSentence(_this) {
        ...
        if (tar.length > 1) {
            $.Alert('Only one segment can be selected');
            return;
        }
        if (tar.hasClass("repeated") || tar.hasClass("locked")) {
            $.Alert("This segment cannot be operated");
    ...
    function lock() {
        ...
        $.LoadTips("Locking checked segments");
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    $.LoadTips("Locking successful, refreshing");
                    window.setTimeout(() => {
                        // loadPage.initPage(sign, fid);
                        localParam.isFirst = true;
                        if (sign){
    ...
    function unlock(flag) {
        ...
        if (flag) {
            ...
            isAll = true;
            isConfirm = confirm("Unlock all?");
        }
        if (!isConfirm) return;
        $.LoadTips("Unlocking segments");
        $.ajax({
            url: urls.editUrl + "/Lock/BatchLockState",
            method: "post",
            data: JSON.stringify({
            ...
            success: function (result) {
                ...
                if (result.success) {
                    ...
                    $.LoadTips("Unlocking successful, refreshing");
                    window.setTimeout(() => {
                        // loadPage.initPage(sign, fid);
                        localParam.isFirst = true;
                        if (sign){
    ...
    function copySpelling(newtxt, wid, oldtxt, fid, pid) {
        ...
        if (_wid === wid) {
            ...
            params.PackID = [pid];
            $.LoadTips("Copying check results");
            $.ajax({
                method: "post",
                url: url,
                dataType: "json",
            ...
            $.Alert("Segments not matching");
        }
    }

    //一键翻译
    function rootTranSource() {
        $.Confirm('Are you sure to pre-translate?');
        $('#my-confirm').modal({
            onConfirm: function (e) {
                var PackID = [];
                for (let i = 0; i < dataConfig.dataList.length; i++) {
                ...
                $.LoadTips("Translation in progress, do not operate repeatedly");
                $.ajax({
    ...
    function tMSource() {
        ...
        $('input[name=matching]').on('blur',function () {
            ...
            if (matchQA.val() < 75 || matchQA.val() > 100) {
                $.Alert('The matching degree should be set at 75% - 100%');
                return false;
            }
        });
    ...
    function batchCopySource(){
        ...
        $.LoadTips('Copy in progress, do not operate repeatedly');
        $.ajax({
            ...
            success: function (data) {
                $(".my-loading").remove();
                if (data.success) {
                    $.LoadTips("Copy successful, refreshing");
                    window.setTimeout(function () {
                        localParam.isFirst = true;
    ...
    function rootCopySource() {
        ...
        $.Confirm('Are you sure to copy all sources to targets?');
        $('#my-confirm').modal({
            onConfirm: function (e) {
                $.LoadTips('Copy in progress, do not operate repeatedly');
                $.ajax({
    ...
    function batchDelTarget(one){
        ...
        !one && $.LoadTips('Deleting, do not operate repeatedly');
        $.ajax({
            ...
            success: function (data) {
                if (!one) {
                    $(".my-loading").remove();
                    if (data.success) {
                        $.LoadTips("Deletion successful, refreshing");
                        window.setTimeout(function () {
                            localParam.isFirst = true;
                            let tr = $("tr.table-row.selected").eq(0);
                            switch (localParam.getContentMethod) {
    ...
    function replaceLookup(jumpId,trs) {
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                $("#excFindBtn").attr("disabled", true).html('Finding<i class="am-icon-spinner am-icon-spin"></i>');
            },
            complete: function (xhr) {
                localParam.isGetContent =true;
                $('.my-loading').remove();
                $("#excFindBtn").removeAttr("disabled").html(xhr.responseJSON && xhr.responseJSON.success ? "Find the next segment" : "Find again");
            },
            success: function (data) {
...
var dealSearRepObj = (function (window, document, $) {
    ...
    */
    ...
    $("#excBtn").on("click", function () {
        ...
        if ($.trim($(".exc_exchange_content").val()) === ""
            ...
            $.Alert("Enter alternative content");
            return false;
        }
        if (!$("body").attr("data-rep")) {
            $.Alert("Only the segments found can be replaced");
            return;
        }
        var packArr = [];
        var fileArr = [];
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                $(_this).attr("disabled", true);
                $("body").append(`<div class="zeZhao"></div>`);
                $(_this).html('Replacing<i class="am-icon-spinner am-icon-spin"></i>');
            },
            complete: function () {
                $(_this).removeAttr("disabled");
                $(".zeZhao").remove();
    ...
    $("#excAllBtn").on("click", function () {
        ...
        $.ajax({
            ...
            complete: function () {
                $(_this).removeAttr("disabled");
                $(".zeZhao").remove();
                $(_this).html("Replace all");
                $("#excFindBtn").html(`<span class="am-icon-search"></span> Find`).removeAttr('firstClick');
            },
            success: function (data) {
                if (data.success) {
                    $('.my-loading').remove();
    ...
    */
    ...
    $("#excFindBtn").on("click", function () {
        if (localParam.getContentMethod !== 4 && $("#excFindBtn").text().trim() !== 'Find'){
            //替换前查找初始化
        ...
        if (!$(this).attr('firstClick')){//第一次查找
            ...
            if (current === $("tr.table-row").length){
                ...
                $.Tips("No content found");
                current = 0;
                $("#excFindBtn").html("Find again").removeAttr('firstClick');
            }else {
        ...
        if (checkList.length > 0) {
            if (text) {
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        $(_this).attr("disabled", true);
                        $(_this).html('<i class="am-icon-spinner am-icon-spin"></i>Searching');
                    },
                    complete: function () {
                        $(_this).removeAttr("disabled");
                        $(_this).html('<span class="am-icon-search"></span> Search');
                    },
                    success: function (data) {
                        if (data.success) {
