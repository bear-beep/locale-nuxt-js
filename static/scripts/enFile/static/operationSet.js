...
var classOperation = (function ($) {
    ...
    function clearContent() {
        ...
        $('.evaluateSentence>.evaluateSen').text('Current segment:');
    ...
    function scrollNextSentence() {
        ...
        var joinTips = `<div class="joinTips" title="Segments can be merged" onclick="dealTranObj.joinFrameSentence()">
                          <i class="am-icon-arrows-v"></i>
                        </div>`;
    ...
    function scrollToNoConfirmSentence(trT) {
        ...
        var isAll = true;
        var flag = trs.toArray().every((item, index) => {
        ...
        if (flag) {
            if ($('tr.init-row').length > 0) {
                ...
                isAll && $.Alert('No unsubmitted segments');
                return false;
            }
        }else {
    ...
    $(document).on("keydown", function (event) {
        ...
        if (e.ctrlKey && e.keyCode === _keyCode._q && $('#qaRunBtn').length > 0) {//qa检查
            ...
            if (!dataConfig.ImportID_TB) {
                $.Alert('Update TM is not added for the current project, the term cannot be added!');
                return
            }
            e.preventDefault();
            ...
            if (!dataConfig.originalEdit) {
                $.Alert("The original text cannot be edited for the current task!");
                return false;
            }
            e.preventDefault();
            ...
            if ($this.attr("data-join") === "true" && !($this.hasClass("locked") || $this.hasClass("repeated"))) {
                $this.find("td.source-text").append(`<div class="joinTips" title="Segments can be merged" onclick="dealTranObj.joinFrameSentence()">
                                                        <i class="am-icon-arrows-v"></i>
                                                    </div>`);
            }
            clearContent();
 ...
 */
    ...
    */
    $('.preview-btn,.preview-translation-btn').on('click', function () {
        if (this.classList.contains("preview-btn")) { // 预览原文
            if (dataConfig.dataList.length > 1) {
                $.Alert("Source preview is not supported because the source contains multiple files!");
                return;
            }
           ...
           if (this.dataset.ajax === 'false') {
            ...
            if (dataConfig.dataList.length > 1) {
                $.Alert("Target preview is not supported because the target contains multiple files!");
                return;
            }
            $.LoadTips('Generating target');
    ...
    */
    //    修改
    $(document).on("click", "#search_btn,#filterClearBtn,#allSendBtn,#preDataBtn,#moreFilterBtn,.term-btn,.refuse-btn,.edit-btn,.delete-btn,.copy-btn,.refreshBtn,.merge-btn,.term_btn,.jump_to,.copy-all-btn,.lock-btn,.corpusSearch-btn,.trans-MT,.trans-TM", function (e) {
        var classlist = this.classList;
        var ele = $('tr.table-row.active'),
            eles = $("tr.table-row.active,tr.table-row.selected");
        var changeSts = new ChangeConfirmStatus();
        if (classlist.contains('lock-btn')) { //批量锁定
            dealTranObj.lock();
        ...
        //    修改结束
            ...
            if ($.trim($(".term_source").val()) === ""
                ...
                $.Alert("Source/Target not filled in");
                return false;
            }
        ...
        // } else if (classlist.contains("refreshBtn") || this.id === "filterClearBtn") { //刷新、清除
        } else if (this.id === "filterClearBtn") { //清除筛选
            $.LoadTips(this.id === "filterClearBtn" ? "Clearing filters" : "Refreshing segments");
            clearContent();
            $(".tmTrans>tbody").empty();
            $(".updataList>tbody").empty();
            let btnID = this.id; // 接收this指向参数
            localParam.searchOringin = "";
            localParam.searchTrans = "";
            if (btnID === "filterClearBtn") {
            ...
            if (jumpNum.trim() === "") {
                $.Alert("Enter a segment");
                return;
            }
            window.clearTimeout(jumpTimer);
            if ($("div").hasClass('my-loading')){
                $.Alert('Do not operate repeatedly, redirecting');
                return
            }
    ...
    $('#language-pair').on('change', function () {
        ...
        +nm > 0 && (source === dataConfig.sourceLan ? $.Tips('Search by source') : $.Tips('Search by target'));
        $(this).attr('data-nm','1');
    });
    ...
    $("table#edition-list").on("mousedown", function (event) {
        ...
        if (!tarEle.hasClass("active") && !$(e.target).hasClass("s-no")) {
            ...
            if (tarEle.attr("data-join") === "true" && !(tarEle.hasClass("locked") || tarEle.hasClass("repeated"))) {
                tarEle.find("td.source-text").append(`<div class="joinTips" title="Segments can be merged" onclick="dealTranObj.joinFrameSentence()">
                                                        <i class="am-icon-arrows-v"></i>
                                                    </div>`);
            }
        }
    ...
    */
    $('table#edition-list').on('click', 'div.edition-source>.tagWrap', function (e) {
        ...
        if ($(_this).hasClass('end')) {
            $.Alert('Click on the tag to insert a number on the left');
            return
        }
        ...
        insertTimer = window.setTimeout(function () {
            ...
            for (let j = 0; j < targetEle.contents().length; j++) {
                ...
                if (node.nodeName == 'SPAN' && $(node).hasClass('tagWrap') && $(node).attr('data-db') == id) {
                    $.Alert('This label already exists in the translation!');
                    return false;
                }
            }
        ...
        if (ele.nodeType === 1 && (ele.nodeName === 'Q' || ele.nodeName === "U" && ele.parentNode.nodeName === "Q")) {
            ...
            if (termTransData.tText) {
                ...
                termDiv = `<div class="termContainer" style="left:${_left};top:${_top};">
                            <ul>
                                <li><span class="rtext" title="${termTransData.tText}">${termTransData.tText}</span> <span onmousedown="dealCatObj.insertTbWord('', '', this)">Insert</span><span onclick="classOperation.copyTermWord(this)">Copy</span></li>
                                <li class="over1">source：<span>${termTransData.base||''}</span></li>
                                <li>type：<span>${termTransData.remark||''}</span></li>
                                <li class="over2">remarks：<span><span title="${termTransData.note}">${termTransData.note||''}</span></span></li>
                            </ul>
                          </div>`;
                $('body').append(termDiv);
            }
    ...
    $("tbody.qaList").on("click", function (event) {
        ...
        if (e.target.nodeName === "TD" && e.target.classList.contains("clickTd")) {
            ...
            if (tr.attr("data-sen") !== 'null') {
                ...
                // var list = [];
                var obj = new Object();
                console.log(tr.attr("data-comparFileId"));
                obj.ProjectID = dataConfig.ProjectID;
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        $.LoadTips('Obtaining the segment');
                    },
        ...
        if (e.target.nodeName === 'SPAN' && e.target.classList.contains("qa-down-span")
            ...
            if (check == '拼写检查' || check == '术语检查') {
                strA = `<dd><label><input type="checkbox" value='${msg}' data-id="${id}" data-fid="${fid}" data-pid="${pid}" data-check="${checkParam}" data-checkName="${check}" data-fileName="${fileName}" data-checkLevel="${checkLevel}" onclick="dealQaObj.ignoreQAResultAll(this,${idx})"/>Ignore All</label></dd>`
            } else {
                strA = '';
            }
            var dlStr = `<dl class="qa-down-list" style="left:${left - 8}px;top:${top + 24}px">
                            <dd>
                                <label><input type="checkbox" value='${msg}' data-id="${id}" data-fid="${fid}" data-pid="${pid}" data-check="${checkParam}" data-checkName="${check}" data-fileName="${fileName}" data-checkLevel="${checkLevel}" onclick="dealQaObj.ignoreQAResult(this,${idx})"/>Ignore</label>
                            </dd>
                            ${strA}
                        </dl>`;
            $("dl.qa-down-list").remove();
...
ChangeConfirmStatus.prototype.single = function (_this) {
    ...
    switch (link) {
        case 'T':
            ...
            confirmTd.attr({ "data-iscon": "0", "title": "Untranslated" });
            break;
        case 'ES':
            confirmTd.find('img').attr('src', iconsrc.yy_no);
            confirmTd.attr({ "data-iscon": "0", "title": "Untranslated" });
        ...
        case 'C':
            ...
            confirmTd.attr({ "data-iscon": "2", "title": "Pending review" });
            break;
        case 'Q':
            confirmTd.find('img').attr('src', iconsrc.qa_no);
            confirmTd.attr({ "data-iscon": "4", "title": "Pending QA" });
            break;
        case 'P':
            confirmTd.find('img').attr('src', iconsrc.pm_no);
            confirmTd.attr({ "data-iscon": "-1", "title": "Being revised by PM" });
            break;
    }
    ...
    tarList.toArray().forEach(function (ele) {
        ...
        switch (link) {
            case 'T':
                ...
                tar.attr({ "data-iscon": "0", "title": "Untranslated" });
                break;
            case 'ES':
                tar.find('img').attr('src', iconsrc.yy_no);
                tar.attr({ "data-iscon": "0", "title": "Untranslated" });
            ...
            case 'C':
                ...
                tar.attr({ "data-iscon": "2", "title": "Pending review" });
                break;
            case 'Q':
                tar.find('img').attr('src', iconsrc.qa_no);
                tar.attr({ "data-iscon": "4", "title": "Pending QA" });
                break;
            case 'P':
                tar.find('img').attr('src', iconsrc.pm_no);
                tar.attr({ "data-iscon": "-1", "title": "Being revised by PM" });
                break;
        }
